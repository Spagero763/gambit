  ...
  return (
    <div className="mx-auto flex min-h-[100dvh] w-full max-w-2xl flex-col px-5 py-5">
      <div className="flex items-center justify-between">
        <Link
          href="/"
          className="inline-flex items-center gap-2 rounded-full glass px-3 py-1.5 text-sm text-ink-dim"
        >
      ...
      <div className="mt-4 flex items-center gap-3">
        {players.map((p, i) => (
          <div
            key={i}
            className={cn(
              "flex flex-1 items-center gap-2.5 rounded-2xl px-3 py-2.5 transition-all",
              p.active ? "glass ring-1 ring-white/15" : "bg-white/[0.03]"
            )}
          >
            <span
              className={cn(
                "grid h-8 w-8 place-items-center rounded-lg bg-white/5 font-display text-base font-bold",
                p.accent
              )}
            >
              {p.mark}
