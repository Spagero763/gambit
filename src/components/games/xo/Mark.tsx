...
export function Mark({ kind }: { kind: "X" | "O" }) {
    ...
    return (
      <svg viewBox="0 0 100 100" className="h-full w-full p-4">
        <motion.line
          ...
          stroke="#aaa7ff" strokeWidth="11" strokeLinecap="round"
  ...
  return (
    <svg viewBox="0 0 100 100" className="h-full w-full p-4">
      <motion.circle
        cx="50" cy="50" r="28"
        fill="none" stroke="#3ecf8e" strokeWidth="11" strokeLinecap="round"
        initial={{ pathLength: 0, rotate: -90 }}
        animate={{ pathLength: 1 }}
        transition={{ duration: 0.36, ease: "easeOut" }}
        style={{ transformOrigin: "center", filter: "drop-shadow(0 1px 2px rgba(0,0,0,0.5))" }}
      />
    </svg>
  );
}
